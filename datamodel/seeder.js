...
module.exports = (listesService, articlesServices) => {
    return new Promise(async (resolve, reject) => {
            try {
                await listesService.dao.db.query("CREATE TABLE listes (id SERIAL PRIMARY KEY, namelistes TEXT, date DATE, archived BOOLEAN, deleted BOOLEAN)")
                ...
                for (let i = 0; i < 5; i++) {
                    const listes_id =  await listesService.dao.insert(new Listes("listes" + i,
                        new Date(+(new Date()) - Math.floor(Math.random() * 10000000000)), true, false))
                    for (let j = 0; j < 5; j++){
